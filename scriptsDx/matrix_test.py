...
def main():
    ...
    print(f"\nResults Breakdown:")
    print(f"  ✅ PASSED: {passed} ({passed*100//len(matrix_results)}%)")
    print(f"  ❌ FAILED: {failed} ({failed*100//len(matrix_results) if len(matrix_results) > 0 else 0}%)")
    ...
    if failed == 0 and errors == 0 and timeout == 0:
        ...
        print(f"⚠️  ISSUES FOUND - {failed + errors + timeout} tests had problems")
    print("=" * 100)
