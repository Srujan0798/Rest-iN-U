...
- [END OF VOLUME 1.6: TITAN DEEP INTERNALS - APPLICATION SECURITY MECHANICS](#application-security)
...
- [END OF VOLUME 1.7: TITAN GEMINI RESEARCH - ADVANCED ATTACK PATTERNS](#patterns)
...
- [END OF VOLUME 3: TITAN GEMINI RESEARCH - SUPPLY CHAIN SECURITY](#supply-chain-security)
...
## VOLUME 8: TITAN SECURITY PROTOCOLS

> **The "Fort Knox" Strategy**
...
graph TD
A[Start] --> B{Public Web App?}
B -- Yes --> C{SPA or SSR?}
C -- SSR (Next.js) --> D[HttpOnly Cookies (Session)]
C -- SPA (React) --> E{High Security?}
E -- Yes --> F[BFF (Backend for Frontend) + Cookies]
E -- No --> G[Access Token (Memory) + Refresh Token (HttpOnly Cookie)]
B -- No --> H{Mobile App?}
H -- Yes --> I[OAuth 2.0 + PKCE]
B -- No --> J{M2M / API?}
J -- Yes --> K[Client Credentials / API Keys]

```text

**The Titan Rule:**

- **Web**: Always use **HttpOnly Cookies**. LocalStorage is for preferences, NOT tokens (XSS risk).
- **Mobile**: Use **PKCE**.
- **API**: Use **Signed API Keys** or **mTLS** for internal services.

### DEEP DIVE: ZERO TRUST (Never Trust, Always Verify)

> **The Philosophy**: "The call is coming from inside the house."

**The Old Way**: "I'm inside the VPN/VPC, so I'm safe."
**The Titan Way**: "I don't trust you, even if you are `localhost`."

**Implementation**:

1. **Identity**: Every request must have a token (JWT/mTLS). No "allow all" from internal IPs.
2. **Device**: Is the device managed? Is it healthy? (CrowdStrike/SentinelOne).
3. **Network**: Micro-segmentation. Service A can ONLY talk to Service B on port 443. Deny all else.

### DEEP DIVE: OWASP TOP 10 2024 (The New Threats)

1. **Broken Access Control**: The king. "Can User A see User B's data?" (IDOR).
- *Fix*: Middleware that checks `resource.ownerId === user.id`.
2. **Cryptographic Failures**: Storing passwords in plain text or using `md5`.
- *Fix*: Use `bcrypt` or `argon2`. Rotate keys.
3. **Injection**: SQLi is still here.
- *Fix*: ORM (Prisma) or Parameterized Queries.

### TITAN SECURITY CHECKLIST

> **The "Don't Get Hacked" List**

- [ ] **Auth**: MFA enforced for admins?
- [ ] **Data**: DB encrypted at rest (AES-256)?
- [ ] **Transport**: TLS 1.3 only? (No SSLv3).
- [ ] **Headers**: `Helmet` enabled? (CSP, HSTS, X-Frame).
- [ ] **Deps**: `npm audit` clean? Snyk scanning?
- [ ] **Secrets**: No `.env` in git? (Use Vault/AWS Secrets).
- [ ] **Logs**: PII redacted from logs?
