...
- [Data Engineering & Big Data Architecture](#data-engineering--big-data-architecture)
...
- [TITAN IMPLEMENTATION: Idempotent ETL Pipeline](#titan-implementation-idempotent-etl-pipeline)
- [VOLUME 6: THE INFINITE (The "Future")](#volume-6-the-infinite-the-future)
- [VOLUME 7: TITAN PRODUCTION SCARS (Incidents & Post-Mortems)](#volume-7-titan-production-scars-incidents--post-mortems)
- [Incident #10.1: The 'Thundering Herd' Kafka Rebalance](#incident-101-the-thundering-herd-kafka-rebalance)
- [Incident #10.2: The 'Silent Corruption' WAL Overflow](#incident-102-the-silent-corruption-wal-overflow)
- [Incident #10.3: The 'Data Skew' OOM](#incident-103-the-data-skew-oom)
- [Titan Data Engineering Checklist (The 'Gold Standard')](#titan-data-engineering-checklist-the-gold-standard)
- [TITAN: Advanced Big Data Concepts](#titan-advanced-big-data-concepts)
- [VOLUME 8: THE TITAN DATA ENGINEERING MANIFESTO](#volume-8-the-titan-data-engineering-manifesto)
...
> **Status**: TITAN (In Progress)
> **Target**: 50,000 Lines
> **Coverage**: Spark, Airflow, Kafka, Debezium
...
{% if is_incremental() %}
WHERE_loaded_at > (SELECT MAX(_loaded_at) FROM {{ this }})
{% endif %}
),
...
### TITAN: Spark Optimization Deep Dive

#### The Shuffle Sort (The Performance Killer)

Shuffling occurs during `groupBy`, `join`, and `repartition`. It involves writing data to disk and transferring it over the network.

- **Shuffle Spill**: Occurs when data doesn't fit in the executor's memory. Result: 10x-100x slowdown.
- **Fix**: Increase `spark.memory.fraction` or use `Broadcast Joins` for small tables to eliminate the shuffle entirely.

#### Data Skew & Salting

When one key (e.g., `user_id = 0` for guests) has millions of rows while others have hundreds, the executor processing that key becomes a bottleneck.

- **Salting Technique**: Append a random integer to the key before joining/aggregating, then aggregate again without the salt.

```python
# Spark Salting Pattern
from pyspark.sql import functions as F

# Add salt to distribute skewed keys
df_salted = df.withColumn("salt", (F.rand() * 10).cast("int"))
df_salted = df_salted.withColumn("salted_key", F.concat(F.col("user_id"), F.lit("_"), F.col("salt")))

# Aggregate with salt
intermediate_df = df_salted.groupBy("salted_key").agg(F.sum("amount").alias("batch_sum"))

# Final aggregate (remove salt)
final_df = intermediate_df.withColumn("user_id", F.split(F.col("salted_key"), "_")[0]) \
    .groupBy("user_id").agg(F.sum("batch_sum").alias("total_amount"))
```text

## TITAN: Airflow Expert Patterns

### Dynamic DAG Generation (The Right Way)

Generating tasks dynamically is powerful, but doing it at **parse time** (top-level code) kills the scheduler.

- **Vibe**: `for i in range(1000): Task(...)` in the DAG file.
- **Titan**: Use `Dynamic Task Mapping` (Airflow 2.3+) to expand tasks at **runtime** based on the output of a previous task.
...
### TITAN: Kafka Exactly-Once Semantics (EOS)

Achieving exactly-once is the "Holy Grail" of data engineering.
- **Idempotent Producer**: Set `enable.idempotence=true`. Kafka assigns a sequence number to every batch, preventing duplicates on retry.
- **Transactional Writes**: Use `initTransactions()`, `beginTransaction()`, and `commitTransaction()`. This ensures that offsets are only committed if the downstream write (e.g., to a DB) succeeds.

#### Data Quality Circuit Breakers
Don't just log errors; stop the pipeline if data is garbage.
- **Great Expectations**: Define "Expectations" (e.g., `expect_column_values_to_be_between`).
- **Circuit Breaker**: If the "Expectation Suite" fails, the pipeline raises an exception and prevents the `Load` stage from executing, saving the warehouse from corruption.

```python
# Data Quality Gate
validation_result = batch.validate(expectation_suite)
if not validation_result.success:
send_pagerduty_alert("Data Quality Failure: Revenue mismatch detected!")
raise PipelineCircuitBreaker("Stopping pipeline to prevent data corruption.")
```text
