    ...
    async analyzeProperty(propertyId: string, data: ClimateData): Promise<any> {
        ...
        const analysis = await prisma.climateAnalysis.upsert({
            ...
            create: {
                ...
                overallRiskScore,
                riskGrade,
                floodRiskScore: floodRisk.riskScore,
                fireRiskScore: wildfireRisk.riskScore,
                earthquakeRiskScore: seismicRisk.riskScore,
                hurricaneRiskScore: stormRisk.hurricaneRisk,
                tornadoRiskScore: stormRisk.tornadoRisk,
                droughtRiskScore: extremeWeatherRisk.droughtRisk,
                heatRiskScore: extremeWeatherRisk.riskScore,
                floodZone: data.floodZone,
                floodZoneDescription: floodRisk.factors.join(', ') || null,
                seismicZone: seismicRisk.riskScore > 50 ? 'High' : seismicRisk.riskScore > 30 ? 'Moderate' : 'Low',
                historicalEvents: projections,
                insuranceNotes: [insuranceAnalysis.status],
                estimatedPremiumImpact: insuranceAnalysis.current > 3000 ? 'VERY_HIGH' : insuranceAnalysis.current > 2000 ? 'HIGH' : insuranceAnalysis.current > 1500 ? 'MODERATE' : 'LOW',
